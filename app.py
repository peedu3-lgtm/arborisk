...
LANG = {
    "ET": {
        ...
        "weather": "Tuul (m/s)", "temp": "Temp/Ilm", "county": "Vali maakond", "tree": "Puu liik ja seisund",
        ...
        "hazards_base": [
            ...
            ["Vale kukkumissuund", "Langetamine", 1, 5], ["Müra/Vibra", "Tervisekahjustus", 3, 2],
            ["Isikud/Autod", "Vigastusoht", 2, 3], ["Vara (aiad)", "Löökkahjustus", 2, 3],
    ...
    "EN": {
        ...
        "weather": "Wind (m/s)", "temp": "Temp/Weather", "county": "Select county", "tree": "Tree species & condition",
        ...
        "hazards_base": [
            ...
            ["Wrong direction", "Felling", 1, 5], ["Noise/Vibra", "Health risk", 3, 2],
            ["Bystanders/Cars", "Injury risk", 2, 3], ["Property (fences)", "Impact damage", 2, 3],
...
st.title(f"🌳 Arborisk Pro v6.4 ({lang_choice})")
...
with col_c: puu_seisund = st.text_area(T['tree'], placeholder="Mädanik, kalle...")
with col_d: keskkond_info = st.text_area(T['env'], placeholder="Tee lähedus, kalle...")
...
for i, oht in enumerate(T["hazards_base"]):
    with st.expander(f"📍 {oht[0]}", expanded=False):
        ...
        with c1: kirj = st.text_input(T['desc'], value=oht[1], key=f"k{i}")
        with c2:
            v_val = st.multiselect(T['tools'], T['tools_list'], key=f"v{i}")
            v_lisa = st.text_input(T['custom_tools'], key=f"vl{i}")
            v_kokku = ", ".join(v_val) + (f" {v_lisa}" if v_lisa else "")
        with c3:
            m_val = st.multiselect(T['measures'], T['measures_list'], key=f"m{i}")
...
def loe_pdf():
    ...
    pdf.cell(0, 4, enc("T (1-5) x R (1-5) = Score. 1-4: Low, 5-12: Med, 15-25: High"), ln=True)
